...
// ── (NovoPostoDialog moved to ./novo-posto-dialog.tsx) ───────────────────────
...
export default function LocaTechDashboard() {
  ...
  if (!selected && allStations.length === 0) {
    return (
        ...
        <main className="flex-1 overflow-y-auto flex flex-col">
          ...
          <div className="flex-1 p-4 space-y-4 max-w-lg mx-auto w-full">
            <div className="bg-white rounded-3xl p-8 text-center shadow-sm border border-slate-100">
              ...
              <p className="text-slate-700 font-bold text-sm">Nenhum posto encontrado</p>
              <p className="text-slate-400 text-xs mt-1">Ainda não há postos disponíveis perto de si.</p>
              {(userRole === 'GESTOR' || userRole === 'ADMIN') && (
                <motion.button
                  onClick={() => setShowNovoPosto(true)}
                  whileHover={{ scale: 1.04 }}
                  whileTap={{ scale: 0.96 }}
                  className="mt-4 px-5 py-2 bg-[#0d1b3e] text-white text-sm font-bold rounded-full hover:opacity-90 transition-opacity"
                >
                  <IPlus className="w-4 h-4 inline mr-1" /> Adicionar Novo Posto
                </motion.button>
              )}
            </div>
          </div>
